 ...
 */
function callback_noticebd_uses_sections() {
    return true;
}
 ...
 * @return bool Returns true
 */
function callback_noticebd_load_content(&$navigation, $course, $coursenode) {
    return $navigation->load_generic_course_sections($course, $coursenode, 'noticebd');
}
 ...
 * @return string
 */
function callback_noticebd_definition() {
    return get_string('topic');
}

function callback_noticebd_get_section_name($course, $section) {
    // We can't add a node without any text
    if ((string)$section->name !== '') {
        return format_string($section->name, true, array('context' => get_context_instance(CONTEXT_COURSE, $course->id)));
    } else if ($section->section == 0) {
        return get_string('section0name', 'format_noticebd');
    } else {
 ...
 * @return stdClass
 */
function callback_noticebd_ajax_support() {
 ...
 */
function callback_noticebd_ajax_section_move($course) {
    global $COURSE, $PAGE;
